...
ax1.plot([-4, 4],[0,0], c='black')
ax1.plot([-4, 4],[2, 2], c='black')
ax1.plot([-4, 4],[4, 4], c='black')
ax1.plot([-4, 4],[-2, -2], c='black')
ax1.plot([-4, 4],[-4, -4], c='black')

ax2 = fig.add_subplot(2, 1, 2)
ax2.plot([0,NUM],[3.1416,3.1416], color='black')
...
for i in range(1, NUM):
    ...
    if intersected == True:
        color = 'green' #Las agujas que intersecten con las líneas se pintaran de verde.
        num_int = num_int + 1.0
        ratio[i] = num_int/i
    else:
        color = 'red' #Las agujas que NO intersecten con las líneas se pintaran de rojo.
        ratio[i] = num_int/i


    ax1.title.set_text("Numero de agujas lanzadas: {}".format(i))
    ...
    ax2.title.set_text("π ≈ {}".format(2/ratio[i]))
    plt.pause(0.0000001)
